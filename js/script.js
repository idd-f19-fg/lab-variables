...
let jobTitle = `Swimming instructor and lifeguard`;
let program = `Interaction Design and Technology System`;
let geolocation = `Toronto`;
let annSalary = `11000`;
let companyName = `Davina Swim House`;
console.log(`${firstName}, will be an ${jobTitle} in ${geoLocation},making ${11000} for ${companyName}`);
...
let futureYear = 2092;
let age = (1992, 2092); {
let birthYear = 1992 - 2092;
let futureYear =(1992 - 2092) - 1;
let bothAge = 1992 + 2092;
}
console.log(`I will be either ${ageCalculator}` + age(futureYear, birthYear) + `in` + futureYear);
...
let maxAge = 90;
let perDay = 4;
let yearsLeft = maxAge - ageNow;
let yearsLeft = yearsLeft = perDay;
console.log(`you will need ${lifeSupply} to last you untilthe ring old age of ${maxAge}`);
...
let circleRadius = 4;
let circumFerence = Math.Pi * 2 * circleRadius;
let area = Math.PI * circleRadius * circleRadius;
console.log(`The circumFerence is ${Geometrizer}` + circumFerence);
